...
public class StatePopulations {
	...
	public static void main(String[] args) throws IOException {
		...
		String stateAbbriv = null;

		for (i = 0; i < zipCodeState.size(); ++i) {
			// TODO: Using ZIP code, find state abbreviation
			if (myZipCode == zipCodeState.get(i).getValue1()) {
				stateAbbriv = zipCodeState.get(i).getValue2();
				System.out.println(zipCodeState.get(i).getValue2());
			}
		}

		for (i = 0; i < abbrevState.size(); ++i) {
			// TODO: Using state abbreviation, find state name
			if (stateAbbriv.equals(abbrevState.get(i).getValue1())) {
				stateAbbriv = abbrevState.get(i).getValue2();
				System.out.println(abbrevState.get(i).getValue2());
			}
		}

		for (i = 0; i < statePopulation.size(); ++i) {
			// TODO: Using state name, find population. Print pair info.
			if (stateAbbriv.equals(statePopulation.get(i).getValue1())) {
				//stateAbbriv = abbrevState.get(i).getValue2();
				System.out.println(statePopulation.get(i).getValue2());
			}
		}
	}
}